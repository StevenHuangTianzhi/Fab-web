...
function handleResult(resultData) {
    ...
    for (let i = 0; i < Math.min(1, resultData.length); i++) {
        ...
        rowHTML += "<th>"
            for (let j = 0; j < resultData[i]["movie_titles"].length; j++)
            	rowHTML += '<a href="single-movie.html?id=' + resultData[i]['movie_ids'][j] + '">'
            	+ resultData[i]["movie_titles"][j]+ ', ' + '</a>';
...
$("#return").submit((event)=>addHandler(event));